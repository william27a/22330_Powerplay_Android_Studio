...
public class Global {
    ...
    public static boolean isClose(DcMotor motor, double tolerance) {
        ...
        if ((current < target && (current + tolerance) > target)
                || (current > target && (current - tolerance) < target)) {
            return true;
        }
        return false;
    }
    ...
    public static double coneHeightFromStack(int cones) { return ((cones-1) * CONE_TO_CONE_HEIGHT) + 7; }
